...
export default {
  ...
  methods: {
    /**
     * Set the position of the confetti
     * @returns void
     */
    setPosition() {
      const confettiSvg = document.getElementById(`confetti-${this.index}`);
      console.log(this.index);
      if (this.index % 2 == 0) {
        confettiSvg.style.left = `${this.position.x}px`;
        confettiSvg.style.top = `${this.position.y}px`;
      } else {
        confettiSvg.style.right = `${this.position.x}px`;
        confettiSvg.style.top = `${this.position.y}px`;
        confettiSvg.style.transform = "rotateY(-180deg)";
      }
      confettiSvg.style.width = "40%";
      confettiSvg.style.position = "absolute";
    },
  },
};
</script>
